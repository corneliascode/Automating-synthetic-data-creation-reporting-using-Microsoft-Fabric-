...
# #### This notebook trains & evaluate a decision tree regressor for regression tasks following the next steps:
# 
...
# - Evaluates model performance by calculating common metrics like Mean Absolute Error (MAE), Mean Squared Error (MSE), and R-squared score.
...
def train_decision_tree(X_train, y_train, max_depth=5):
    ...
    feature_importances = []
    for name, importance in zip(X_train.columns, tree_regressor.feature_importances_):
        feature_importances.append({'name': name, 'importance': importance})
    return tree_regressor, feature_importances
...
def evaluate_model(y_test, y_pred):
  """
  Evaluates the performance of a model and returns metrics as a dictionary.
  ...
  metrics = {"mean_absolute_error": mae, "mean_squared_error": mse, "r2_score": r2}
  return metrics
...
tree_regressor, feature_importances = train_decision_tree(X_train, y_train, max_depth=5)
prediction = make_predictions(tree_regressor, X_test)
...
with open(text_file_path, 'w') as f:
    f.write('---- Decission tree regressor metrics ----\n\n')
    for each in metrics:
        f.write(f"{each}: {metrics[each]}\n")
    f.write('\n')
    f.write('---- Decission tree regressor features importance ----\n\n')
